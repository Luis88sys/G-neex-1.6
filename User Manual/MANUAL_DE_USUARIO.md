...
- **Importar respaldo:** Elija el JSON recibido, confirme y siga el aviso en pantalla; sustituye los datos de trabajo de esta instancia.
...
- Campos adicionales: **número de PO / orden de compra** (obligatorio), **remito**, **proveedor**.
...
Al pulsar **Recepción parcial** o **Recepción total (pendiente)**, la aplicación cambia a la pestaña **Movimientos**, selecciona **Compra de stock** y rellena el formulario (mismo que un pedido manual). Debe revisar y pulsar **Procesar movimiento**. El stock entra al confirmar ese movimiento. Si modifica artículo o cantidad de forma incompatible con la línea, la compra se guarda igual pero **no** se vincula a esa línea (aparece un aviso).
...
- **Reporte de carga por camión**: En el detalle del camión puede usar **Exportar reporte de carga** (XLSX) e **Imprimir reporte de carga** con materiales, cantidades y dimensiones cargadas.
...
> Las capturas de pantalla de este manual se obtienen de la build v1.6; para volver a generarlas, véase `docs/app-screenshots/README.md`.

## Acerca del autor

La aplicación es obra de **Luis Goire**, quien la desarrolla como aficionado a la programación y con la idea de seguir creciendo en el oficio de programador. En la interfaz aparece un crédito discreto (nombre; al pasar el cursor, una breve descripción).
