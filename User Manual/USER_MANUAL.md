...
- **Import backup:** Choose the JSON file, confirm, and follow the on-screen notice; it replaces the working data for this copy.
...
- Extra fields: **PO / purchase order number** (required), **packing slip**, **supplier**.
...
When you tap **Partial receipt** or **Full receipt (remaining)**, the app switches to **Movements**, selects **Stock purchase**, and fills the same form as a manual order. Review and click **Process movement**. Stock is updated when that movement is saved. If you change item or quantity in a way that does not match the line, the purchase still saves but **may not** link to that line (a warning is shown).
...
- **Truck cargo report**: In the truck detail you can use **Export cargo report** (XLSX) and **Print cargo report** with loaded materials, quantities, and dimensions.
...
> Screenshots in this manual match G-NEEX 1.6. To regenerate them, see `docs/app-screenshots/README.md`.

## About the author

The application is created by **Luis Goire**, who builds it as a programming enthusiast and with the goal of growing into a developer role. The UI includes a subtle credit (name; hover for a short note).
