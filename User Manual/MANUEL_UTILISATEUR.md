...
- **Importer la sauvegarde** : choisissez le JSON, confirmez et suivez l’alerte affichée ; cela remplace les données de travail de cette instance.
...
Onglet **Mouvements** (➕) : la grille de types et, après choix, le formulaire en fenêtre superposée.
...
- Champs supplémentaires : **numéro de BC / commande** (obligatoire), **bon de livraison**, **fournisseur**.
...
En cliquant sur **Réception partielle** ou **Réception totale (reste)**, l'application passe à l'onglet **Mouvements**, sélectionne **Achat de stock** et préremplit le formulaire (identique à une commande manuelle). Vérifier puis cliquer sur **Traiter le mouvement**. Le stock est mis à jour à l'enregistrement. Si l'article ou la quantité ne correspond plus à la ligne, l'achat est quand même enregistré mais **sans** lien vers cette ligne (un avertissement s'affiche).
...
- **Rapport de chargement par camion** : Dans le détail du camion, utilisez **Exporter le rapport de chargement** (XLSX) et **Imprimer le rapport de chargement** avec matériaux, quantités et dimensions chargées.
...
> Les captures d’écran de ce manuel correspondent à G-NEEX 1.6. Régénération : `docs/app-screenshots/README.md`.

## À propos de l’auteur

L’application est réalisée par **Luis Goire**, par passion pour la programmation et dans une perspective de formation vers le métier de développeur. L’interface affiche un crédit discret (nom ; au survol, une courte description).
