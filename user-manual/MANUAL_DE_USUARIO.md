...
Pestaña **Movimientos** (➕ en la barra superior): la cuadrícula de tipos y, al elegir un tipo, el formulario en ventana superpuesta (véanse los pasos siguientes). La pestaña **Recepciones** (🧱) en la barra abre el registro de recepciones en Configuración; el **tipo de movimiento** «Recepción de material» en la rejilla usa el mismo icono **🧱** en la app. Los **18 tipos** se muestran siempre en **rejilla de 6 columnas × 3 filas**; si el ancho de pantalla es reducido, la zona de la rejilla permite **desplazamiento horizontal** para mantener la alineación.
...
| Venta directa | Resta | Opcional |
| Expedición de stock | Resta | Obligatorio |
...
### Venta directa y Expedición de stock

- **Referencias internas:** prefijos de movimiento **VDT** (venta directa) y **EXP** (expedición), cada uno con correlativo de 6 dígitos por tipo (véase la tabla general de referencias).
- **Venta directa:** al **procesar**, el **SO (Sales order)** es obligatorio: texto **SO** + **4 a 6** dígitos (p. ej. `SO100450`). El **ID de proyecto** es opcional. Origen de stock solo **principal / cajas / ubicaciones** (véase el paso de Origen stock).
- **Expedición de stock:** **ID de proyecto** y **PR** obligatorios al procesar; el **PR** debe ser **PR** + **4 a 6** dígitos. Mismas reglas de origen que la venta directa. El icono del tipo en la rejilla es **🚛** (el módulo **Transporte** de la barra superior sigue usando **🚚** — son conceptos distintos).
- En **Historial**, el detalle y las exportaciones pueden mostrar **SO** o **PR** según el tipo; existen filtros dedicados **SO (venta directa)** y **PR (expedición)** (coincidencia parcial de texto).
...
- Ubicación
- ID Proyecto (parcial)
- **SO (venta directa)** (parcial; campo guardado en el movimiento)
- **PR (expedición)** (parcial; campo guardado en el movimiento)
...
- Para **Venta directa**, el **SO** guardado; para **Expedición de stock**, el **PR** guardado
- Quién lo realizó
