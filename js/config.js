...
const ConfigManager = {
  ...
  renderUsersTable() {
    ...
    const byId = new Map();
    for (const id of ["gneex-builtin-1", "gneex-builtin-2", "gneex-builtin-3", "gneex-builtin-4"]) {
    ...
    if (!this._usersTableEventsBound) {
      ...
      wrap.addEventListener("change", e => {
        ...
        if (t.classList.contains("user-perm-toggle")) {
          const userId = t.dataset.userId;
          const perm = t.dataset.perm;
          if (!userId || !perm) return;
          Auth.setUserPerm(userId, perm, t.checked);
    ...
    const permKeys = Auth.PERMISSIONS;
    const rows = users
      .map(u => {
        ...
        const permCells = permKeys.map(pk => {
          const has = isAdmin || !!(u.permissions && u.permissions[pk]);
          const dis = isAdmin ? " disabled" : "";
          const chk = has ? " checked" : "";
          return `<td class="perm-cell"><input type="checkbox" class="user-perm-toggle" data-user-id="${Utils.escapeAttr(u.id)}" data-perm="${pk}"${chk}${dis}></td>`;
        }).join("");
      ...
      .join("");
    const permHeaders = permKeys.map(pk =>
      `<th class="perm-header" title="${Utils.escapeAttr(I18n.t(`perm.${pk}`))}"><span>${this.esc(I18n.t(`perm.${pk}.short`))}</span></th>`
    ).join("");
    wrap.innerHTML = `
  ...
  openModalAtTab(tab) {
    ...
    if (typeof Auth !== "undefined") {
      if (!Auth.isAdmin() && tab === "import") tab = "about";
      if (!Auth.isAdmin() && (tab === "users" || tab === "elevation")) tab = "about";
      const cfgWide = Auth.isAdmin() || Auth.isElevated();
      if (!cfgWide) {
        if (tab === "expirations" && !Auth.hasExpirationConfig()) tab = "about";
        if (tab === "itemedit" || tab === "employees" || tab === "suppliers" || tab === "consumables") tab = "about";
      }
    }
    this.renderItemEditorOptions();
    this.syncExpAlertInput();
  ...
  setupEventListeners() {
    ...
    if (tplBtn) tplBtn.addEventListener("click", () => void InventoryManager.exportInventoryImportTemplateCsv());

    const zipBtn = document.getElementById("export-zip-btn");
    if (zipBtn) zipBtn.addEventListener("click", () => Utils.exportZIP());
    ...
    if (exportMovementsOnlyBtn) {
      exportMovementsOnlyBtn.addEventListener("click", () => void Utils.exportMovementsJSON());
    }
    ...
    if (exportTransportsShippedOnlyBtn) {
      exportTransportsShippedOnlyBtn.addEventListener("click", () => void Utils.exportTransportsJSON({ shippedOnly: true }));
    }
    ...
    document.getElementById("receptions-config-export-btn")?.addEventListener("click", async () => {
      ...
      const ordered = (ReceptionsManager.receptions || []).slice().reverse();
      const filtered = ordered.filter(r => this.receptionRowMatchesFilter(r, q));
      const headers = this._buildReceptionsPrintExportHeaders(filtered);
  ...
  _buildReceptionsPrintExportHeaders(receptions) {
    const list = Array.isArray(receptions) ? receptions : [];
    const maxPackages = list.reduce((m, r) => {
      const n = Array.isArray(r?.dimensionsItems) ? r.dimensionsItems.length : 0;
      return Math.max(m, n);
    }, 0);
    const headers = [
    ...
    for (let i = 1; i <= maxPackages; i++) {
      headers.push(`Paquete ${i} L`, `Paquete ${i} W`, `Paquete ${i} H`);
    }
    return headers;
  },
  ...
  async printReceptionsFiltered(receptions, scopeLabel) {
    ...
    const headers = this._buildReceptionsPrintExportHeaders(list);
    ...
    const rows = list.map(r => {
        ...
        const row = {
          [I18n.t("reception.dateShort")]: Utils.formatDate(r.dateReceived),
          ...
          [I18n.t("reception.dimensionsCol")]: this._formatReceptionDimensionsCell(r),
          [I18n.t("reception.purchaseOrder")]: r.purchaseOrder || "—",
          [I18n.t("reception.supplier")]: r.supplier || "—"
        };
        for (let i = 0; i < unitDims.length; i++) {
          const p = unitDims[i] || {};
          row[`Paquete ${i + 1} L`] = p.L || 0;
          row[`Paquete ${i + 1} W`] = p.W || 0;
          row[`Paquete ${i + 1} H`] = p.H || 0;
        }
        return `<tr>${selectedHeaders.map(h => `<td>${esc(row[h] ?? "")}</td>`).join("")}</tr>`;
      })
      .join("");
    Utils.printHtmlDocument(
      I18n.t("config.receptionsTitle"),
      `${I18n.t("history.filterType")}: ${esc(scopeLabel || I18n.t("history.filterAll"))}`,
      `<table class="inventory-table"><thead><tr>${selectedHeaders.map(h => `<th>${esc(h)}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table>`
    );
  },
  ...
  async saveItemEditor() {
    ...
    if (updated.inventoryConsumable && typeof ConsumableManager !== "undefined" && ConsumableManager.ensureMasterName) {
      ConsumableManager.ensureMasterName(updated.description || updated.code);
    }
    const searchInp = document.getElementById("config-item-search");
