  ...
  th,td{padding:2px 3px}
    ...
    async exportReceptionsXlsx(receptions = [], opts = {}) {
        ...
        for (let i = 1; i <= maxPackages; i++) {
            pkgHeaders.push(`Paquete ${i} L`, `Paquete ${i} W`, `Paquete ${i} H`);
        }
        const rows = list.map(r => {
            ...
            const out = {
                Fecha: r?.dateReceived ? this.formatDateTime(r.dateReceived) : "",
                Proyecto: r?.projectId || "",
                Articulo: r?.itemName || "",
                Categoria: r?.materialCategory || "",
                Cantidad: r?.quantity ?? "",
                Dimensiones: `${fmtDim(d?.L)}x${fmtDim(d?.W)}x${fmtDim(d?.H)}`,
                PO: r?.purchaseOrder || "",
                Proveedor: r?.supplier || "",
                Provisional: r?.provisional ? I18n.t("history.yes") : I18n.t("history.no"),
                PresentacionVidrio: glassPacking
            };
            for (let i = 0; i < maxPackages; i++) {
                const p = unitDims[i] || {};
                out[`Paquete ${i + 1} L`] = fmtDim(p?.L);
                out[`Paquete ${i + 1} W`] = fmtDim(p?.W);
                out[`Paquete ${i + 1} H`] = fmtDim(p?.H);
            }
            return out;
        });
        const headers = [
