...
const MovementManager = {
    ...
    openCompraStockFromOrderLine({ orderLineId, quantity }) {
        ...
        if (!isConsum) {
            ...
            if (item.inventoryConsumable) {
                Utils.showToast(I18n.t("orderLines.msgInventoryConsumableNoReceipt"), "warning");
                return;
            }
        }
    ...
    openCompraStockFromOrderLinesBatch({ supplier, entries }) {
        ...
        for (const e of src) {
            ...
            if (!item || item.inventoryConsumable) continue;
        ...
        if (this.currentType === 'COMPRA_STOCK') {
            ...
            (movement.items || []).forEach(li => {
                ...
                if (inv.inventoryConsumable) {
                    patch.mainStock = Math.max(0, Utils.roundDecimal(Math.abs(parseFloat(li.quantity) || 0)));
                    patch.prodStock = 0;
                    patch.transStock = 0;
                }
