...
const OrderLinesManager = {
  ...
  _orderLineActionsHtml(line, canEdit) {
    let actions = "";
    if (canEdit) {
  ...
  _orderLineTableRowHtml(line, canEdit) {
    const remaining = Math.max(0, line.orderedQty - line.receivedQty);
    const editInactive = line.status === this.STATUS.INACTIVA && canEdit;
    ...
    const actions = this._orderLineActionsHtml(line, canEdit);
    ...
    const rowCb = canBatchReceive
      ? `<label class="orderline-select-cell-label"><input type="checkbox" id="${rowSelId}" class="orderline-select-cb orderline-select-cb--row" data-id="${Utils.escapeAttr(
          line.id
    ...
    return `<tr data-id="${Utils.escapeAttr(line.id)}">
          ...
          <td class="orderline-actions">${actions}</td>
        </tr>`;
  },
   ...
   */
  tryAttachOrderLineReceipt(movement, pen) {
    ...
    if (this._isConsumableLine(line) || pen.consumableReceipt) {
      ...
      if (items.length === 1) {
        it = matchCons(items[0]) ? items[0] : null;
      } else {
        it = items.find(matchCons);
      }
      if (!it) return false;
    ...
    if (items.length === 1) {
      ...
      it = items.find(x => {
        if (!x || x.consumableReceipt) return false;
        if (String(x.itemId ?? "") !== penItemIdNorm) return false;
        const q = Math.abs(parseFloat(x.quantity) || 0);
        if (q <= 0) return false;
        if (qExpected > 0 && Math.abs(q - qExpected) > 1e-6) return false;
        return true;
  ...
  commitBatchReceiptAfterCompra(movement) {
    ...
    for (const rec of batch) {
      ...
      if (this._isConsumableLine(line) || rec.consumableReceipt) {
        const nameNorm = String(line.consumableName || "").trim().toLowerCase();
        item = (movement.items || []).find(it => {
          if (!it || !it.consumableReceipt) return false;
          const b = String(it.description || it.code || "").trim().toLowerCase();
          if (!nameNorm || nameNorm !== b) return false;
          const qi = Math.abs(parseFloat(it.quantity) || 0);
          return Math.abs(qi - qtyExpected) <= 1e-6;
        });
      } else {
        const targetId =
        ...
        item = (movement.items || []).find(it => {
          if (!it || it.consumableReceipt) return false;
          if (String(it.itemId ?? "") !== targetId) return false;
          const qi = Math.abs(parseFloat(it.quantity) || 0);
          return Math.abs(qi - qtyExpected) <= 1e-6;
        });
      }
  ...
  async printFilteredTable() {
    ...
    const table = `<table><thead><tr>${selected.map(h => `<th>${esc(h)}</th>`).join("")}</tr></thead><tbody>${body}</tbody></table>`;
  ...
  render() {
    ...
    const canEdit = typeof Auth === "undefined" || Auth.hasOrderLinesEdit();

    const rowsHtml = filtered.map(line => this._orderLineTableRowHtml(line, canEdit)).join("");
  ...
  setupEventListeners() {
    ...
    document.getElementById("orderlines-purchase-suggestions-list")?.addEventListener("click", e => {
      ...
      if (act === "create") this.createLineFromSuggestion(code);
      else if (act === "remove") this.removeSuggestion(code);
    });

    document.getElementById("orderlines-export-btn")?.addEventListener("click", () => void this.exportCsv());
    document.getElementById("orderlines-print-btn")?.addEventListener("click", () => this.printFilteredTable());
    document.getElementById("orderlines-purge-old-btn")?.addEventListener("click", () => this.purgeReceivedOlderThanOneYear());
