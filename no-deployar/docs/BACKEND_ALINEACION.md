...
| Objeto movimiento (raíz) | `type`, `reference`, `items[]`, `notes`, `projectId`, `salesOrder`, `prNumber`, anexos y demás campos que serializa la SPA | Misma cadena JSON en `phoenix-movements` (sin validación de enum en servidor) | ✅ el API persiste el documento tal cual; la SPA valida formatos (p. ej. `VENTA_DIRECTA` / `EXPEDICION_STOCK` con `salesOrder` `^SO\d{4,6}$` y `prNumber` `^PR\d{4,6}$` en `js/utils.js`) |
