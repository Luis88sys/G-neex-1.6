...
3. **PDF:** `no-deployar\docs\export-user-docs-pdf.ps1` (requiere Edge). Los `.html` bajo `no-deployar/User Manual/` y `no-deployar/Presentation/` deben referenciar capturas con **`../docs/app-screenshots/`** (no `./app-screenshots/` ni `../app-screenshots/` desde esas carpetas) para que la impresión headless resuelva bien las imágenes. Luego volver a ejecutar el sync del punto 2 si regeneraste PDF.
