...
**Movements** (➕) in the top bar: the movement-type grid; choosing a type opens the overlay form (see below). The **Receptions** tab (🧱) opens the receptions register under Settings; the **Material reception** movement type in the grid uses the same **🧱** icon in the app. All **18 types** are shown in a fixed **6-column × 3-row** grid; on narrow screens the grid area allows **horizontal scrolling** so the layout stays aligned.
...
| Direct sale | Subtract | Optional |
| Stock dispatch | Subtract | Required |
...
### Direct sale and Stock dispatch

- **Internal references:** movement prefixes **VDT** (direct sale) and **EXP** (dispatch), each with a 6-digit sequence per type (see the general reference table).
- **Direct sale:** when you **process**, **SO (sales order)** is required: literal **SO** + **4 to 6** digits (e.g. `SO100450`). **Project ID** is optional. Stock source is only **main / boxes / locations** (see Stock source step).
- **Stock dispatch:** **Project ID** and **PR** are required when processing; **PR** must be **PR** + **4 to 6** digits. Same source rules as direct sale. The type icon in the grid is **🚛** (the top-bar **Transport** module still uses **🚚** — different concepts).
- In **History**, detail and exports can show **SO** or **PR** by type; there are dedicated filters **SO (direct sale)** and **PR (dispatch)** (partial text match).
...
- Location
- Project ID (partial)
- **SO (direct sale)** (partial; stored field on the movement)
- **PR (dispatch)** (partial; stored field on the movement)
...
- For **Direct sale**, the stored **SO**; for **Stock dispatch**, the stored **PR**
- Who performed it
