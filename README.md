...
- **Panel — consejo del día:** recuadro fijo **encima** del carrusel del encabezado; **366** variantes según el día civil en **hora local** (incluye 29 feb en bisiesto) y el idioma activo (ES / EN / FR en `js/dashboard-daily-tips-data.js`). El carrusel resume métricas y alertas **sin** repetir el consejo en una diapositiva (`js/dashboard.js`).
- **Movimientos — calculadora de cantidad:** junto a la cantidad de cada línea, **🧮** abre un modal (`js/qty-calculator.js`); la expresión se valida con la misma lógica segura que el cuadro de texto del movimiento (`MovementManager._evaluateQuantityExpression`).
...
### Calculadora de cantidad (movimientos)
Junto a la **cantidad** de cada línea del formulario de movimientos, el botón **🧮** abre un modal (`index.html` + `js/qty-calculator.js`) con operaciones aritméticas seguras (misma evaluación que `MovementManager._evaluateQuantityExpression`). Si el modal no estuviera disponible, la app puede recurrir al cuadro de texto clásico (`movements.qtyCalcPrompt`).
